...
class M2NAFDConnector(AFDConnectorBase):
    def __init__(self, 
        ...
        self.attn_use_multistream = self.config.afd_config.attn_use_multistream if self.config.afd_config is not None else False
        self.ffn_use_multistream = self.config.afd_config.ffn_use_multistream if self.config.afd_config is not None else False
    ...
    def send_attn_output(self, 
        ...
        recv_counts = None
        if self.attn_use_multistream:
            self.send_stream.wait_stream(torch.npu.current_stream())
        with npu_stream_switch(self.send_stream, enabled=self.attn_use_multistream and not self.use_aclgraph):
            ...
            # TODO(jcz): wait stream. Need to remove this.
            if not self.use_aclgraph:
                self.send_stream.synchronize()
        # TODO(jcz): wait stream. Need to remove this.
        if self.attn_use_multistream and self.use_aclgraph:
            torch.npu.current_stream().wait_stream(self.send_stream)
        return recv_counts
    ...
    def recv_ffn_output(self, hidden_states: torch.Tensor, metadata: AFDConnectorMetadata) -> torch.Tensor:
        ...
        with npu_stream_switch(self.recv_stream, enabled=self.attn_use_multistream and not self.use_aclgraph):
    ...
    def send_ffn_output(self, ffn_output: torch.Tensor, metadata: M2NAFDConnectorMetadata):
        ...
        if self.ffn_use_multistream:
            self.send_stream.wait_stream(torch.npu.current_stream())
        with npu_stream_switch(self.send_stream, enabled=self.ffn_use_multistream and not self.use_aclgraph):
            ...
            # TODO(jcz): need to remove this.
            if not self.use_aclgraph:
                self.send_stream.synchronize()
        # TODO(jcz): need to remove this.
        if self.ffn_use_multistream and self.use_aclgraph:
            torch.npu.current_stream().wait_stream(self.send_stream)
    ...
    def recv_attn_output(self, metadata: M2NAFDConnectorMetadata) -> Any: 
        ...
        with npu_stream_switch(self.recv_stream, enabled=self.ffn_use_multistream and not self.use_aclgraph):
            expand_x, dynamic_scales, expert_token_nums, recv_counts, expand_scales = torch_npu.npu_m2n_distribute_recv(
                x=torch.tensor([], dtype=x_type, device='npu'),
                group_ep=self.hccl_comm_name,
                world_size=self.attn_size + self.ffn_size,
                moe_world_size=self.ffn_size,
                ep_rank_id=self.rank,
