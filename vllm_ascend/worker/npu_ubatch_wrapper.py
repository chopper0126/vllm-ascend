...
class UBatchWrapper:
    ...
    def _run_ubatches(self, ubatch_metadata, model) -> torch.Tensor:
        ...
        def _ubatch_thread(results, model, ubatch_metadata):
            print(f"start _ubatch_thread, context.id: {ubatch_metadata.context.id}", flush=True)
            with ubatch_metadata.context:
                print(f"start exec model, context.id: {ubatch_metadata.context.id}", flush=True)
                model_output = model(
                    input_ids=ubatch_metadata.input_ids,
                    positions=ubatch_metadata.positions,
                    intermediate_tensors=ubatch_metadata.intermediate_tensors,
                ...
                print(f"finish exec model, context.id: {ubatch_metadata.context.id}", flush=True)
            print(f"_ubatch_thread,context.id: {ubatch_metadata.context.id}, results: {len(results)}", flush=True)
            results.append((ubatch_metadata.context.id, model_output))
    ...
    def _make_ubatch_metadata(self, ubatch_slices, attn_metadata, input_ids,
        ...
        print(f"_make_ubatch_metadata, ubatch_slices:{ubatch_slices}", flush=True)
        forward_contexts = []
        ...
        print(f"finish create forward_contexts ", flush=True)
    ...
    def __call__(self, *args, **kwargs):
        ...
        # print(f"UBatchWrapper.__call__ 被调用了！", flush=True)
        # print(f"self={self}", flush=True)
        # print(f"type={type(self)}", flush=True)
        # print(f"args_len={len(args)}, kwargs_keys={list(kwargs.keys())}", flush=True)
        # print(f"{'=' * 80}", flush=True)

        # print(f"ttg start get_forward_context", flush=True)
        forward_context = get_forward_context()
        # print(f"ttg finish get_forward_context", flush=True)
        batch_descriptor = forward_context.batch_descriptor
        ubatch_slices = forward_context.ubatch_slices
        aclgraph_runtime_mode = forward_context.cudagraph_runtime_mode

        print(f"exec ubatch wrapper, ubatch_slices: {ubatch_slices}, graph: {aclgraph_runtime_mode}", flush=True)
        ...
        if ubatch_slices is None:
            ...
            if aclgraph_runtime_mode in (CUDAGraphMode.NONE,
                ...
                print(f"ttg exec aclgraph_wrapper", flush=True)
        ...
        if num_tokens not in self.aclgraphs \
            ...
            print(f"start make_ubatch_metadata", flush=True)
            ubatch_metadata = self._make_ubatch_metadata(
                ubatch_slices=ubatch_slices,
                attn_metadata=attn_metadata,
                input_ids=input_ids,
            ...
            print(f"start run_ubatches", flush=True)
            return self._run_ubatches(ubatch_metadata, self.model)
