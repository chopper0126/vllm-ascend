    ...
    int64_t compute_gate)
{
    ...
    if (rank >= expert_rank_size) {
        ...
        expand_x_out = at::empty({base_batch_size, hidden_size}, x.options().dtype(at::kBFloat16));
    }
    ...
    if (rank < expert_rank_size && rank < attention_rank_size) {
        simulate_expert_ids = at::empty({base_batch_size, topk}, x.options().dtype(at::kInt));
        simulate_expert_scales = at::empty({base_batch_size, topk}, x.options().dtype(at::kFloat));
    ...
    EXEC_NPU_CMD(aclnnA2e,
        x, expert_ids, scales,
        base_batch_size, hidden_size, topk, expert_rank_size, attention_rank_size, rank,
        group_ep_ptr, aiv_num, compute_gate,
        expand_x_out, simulate_expert_ids, simulate_expert_scales, atten_batch_size, x_active_mask_out);
...
at::Tensor e2a(
    ...
    int64_t aiv_num)
{
    int32_t base_batch_size = (rank >= expert_rank_size) ? expand_x.sizes()[0] : batch_size;
    
    at::Tensor x_out;
    if (rank < expert_rank_size) {
        x_out = at::empty({1, 1}, expand_x.options().dtype(at::kBFloat16));
    } else {
        x_out = at::empty({base_batch_size, hidden_size}, expand_x.options().dtype(at::kBFloat16));
    }
    ...
    EXEC_NPU_CMD(aclnnE2a,
        expand_x, atten_batch_size,
        base_batch_size, hidden_size, topk, expert_rank_size, attention_rank_size, rank,
        group_ep_ptr, aiv_num,
        x_out);
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.def(
        "a2e(Tensor x, Tensor? expert_ids=None, Tensor? scales=None, "
    ...
    ops.impl("a2e", torch::kPrivateUse1, &vllm_ascend::a2e);

    ops.def(
        "e2a(Tensor expand_x, Tensor atten_batch_size, "
        "    int batch_size=0, int hidden_size=0, int topk=0, "
        "    int expert_rank_size=0, int attention_rank_size=0, int rank=0, "
        "    str group_ep='', int aiv_num=0) -> Tensor"
    );
    ops.impl("e2a", torch::kPrivateUse1, &vllm_ascend::e2a);
