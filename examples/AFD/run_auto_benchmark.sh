...
BSIZE_LIST="24"
DP=8
DATA_MULTIPLIER=1 #数据量
# TIMES=4 # bs > 160 ,并发太小导致触发不了dbo,导致触发不了入图

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_PY="/home/y00889327/benchmark/ais_bench/datasets/synthetic/synthetic_config.py"
MODEL_PY="/home/y00889327/benchmark/ais_bench/benchmark/configs/models/vllm_api/vllm_api_stream_chat.py"
...
for BSIZE in $BSIZE_LIST; do
    ...
    RUN_DIR="${ARCHIVE_ROOT}/BSIZE_${BSIZE}_${TIMESTAMP}"
    
    

    log "启动后端服务..."
    # 添加日志目录参数 -l (如果内部脚本支持)
    bash "${SCRIPT_DIR}/8A8F_attention.sh" -d 0,1,2,3,4,5,6,7 -s $BSIZE -l "${RUN_DIR}/log" &
    PID_ATT=$!
    
    bash "${SCRIPT_DIR}/8A8F_ffn.sh" -d 8,9,10,11,12,13,14,15 -s $BSIZE -l "${RUN_DIR}/log" &
    PID_FFN=$!
    ...
    FFN_SRC="/home/y00889327/prof_ffn"
    if [ -d "$FFN_SRC" ]; then
        # 获取最新的模式串
